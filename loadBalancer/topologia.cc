...
int main (int argc, char *argv[]) 
{
  ...
  plot1.SetLegend("Numero clientes","Paquetes recibidos");
  Gnuplot2dDataset datos1("paquetes_recibidos");
	datos1.SetStyle (Gnuplot2dDataset::LINES_POINTS);
    
  Gnuplot plot2;
  plot2.SetTitle ("Grafica2");
  plot2.SetLegend("Numero clientes","Tiempo entre paquetes (ms)");
  Gnuplot2dDataset datos2("tiempo_entre_paquetes");
  datos2.SetStyle (Gnuplot2dDataset::LINES_POINTS);

  Gnuplot plot3;
  plot3.SetTitle ("Grafica3");
  plot3.SetLegend("Numero de servidores","Paquetes recibidos");
  Gnuplot2dDataset datos3("paquetes_recibidos");
  datos3.SetStyle (Gnuplot2dDataset::LINES_POINTS);
  
  Gnuplot plot4;
  plot4.SetTitle ("Grafica4");
  plot4.SetLegend("Numero de servidores","Tiempo entre paquetes (ms)");
  Gnuplot2dDataset datos4("tiempo_entre_pqts");
  datos4.SetStyle (Gnuplot2dDataset::LINES_POINTS);
    
  int init_clients = client_number;
  int init_servers = server_number;   
  //Gnuplot2dDataset datos_info;     
  //Gnuplot2dDataset datos_info2;     
  //Gnuplot2dDataset datos_info3;     
  ...
  for(int cont = 0; cont < 2; cont++) {
    ...
    /*  if(j<(iteraciones/2)) {
      ...
      std::cout << "Numero clientes : " << client_number << "\n";
      std::cout << "Numero servidores : " << server_number << "\n";      
      ...
      if(cont == 0) {
        datos1.Add(client_number,avg_total_received.Mean());  
        datos2.Add(client_number,avg_time_delay.Mean());
      }
      else {      
        datos3.Add(server_number,avg_total_received.Mean());
        datos4.Add(server_number,avg_time_delay.Mean());
      }    
    ...
    std::ofstream fichero4("proyectopsr-gf4.plt");
    plot4.GenerateOutput (fichero4);
