...
namespace ofi {
      ...
      NS_LOG_COMPONENT_DEFINE("ip-random");

      IpRandomController::IpRandomController(int server_number) 
      {
	      num_servers = server_number;
      }

      void IpRandomController::ReceiveFromSwitch(Ptr<OpenFlowSwitchNetDevice> swtch, ofpbuf* buffer) 
      {
          ...
          int port = ntohs(opi->in_port);				// Almacena el puerto por el que se ha recibido la trama
		      ...
		      uint32_t src_integer = src_ipv4Addr.Get();			// Dirección IP origen guardada como entero
	          ...
	          if (dst_macAddr.IsBroadcast ())
	          {
	          	NS_LOG_LOGIC ("Paquete difusión MAC");
             		...
             		{
             			NS_LOG_LOGIC ("Paquete ARP");
                 		/* Antes de proceder a la elección de manera aleatoria se tiene en cuenta si la IP origen es par o impar. En caso de ser par
                 		 ĺa petición será atendida aleatoriamente por uno de los servidores conectadios a puertos pares, mientras que si 
                 		 es impar se procederá de manera análoga pero con los servidores impares
             			*/
             			UniformVariable uv;						// Variable aleatoria

                 		if (src_integer % 2 == 0)		// IP origen par
                 		{
                 			// Generamos aleatoriamente un número PAR entre 0 y num_servers
                 			out_port = (2*uv.GetInteger(0, num_servers))%num_servers;		// Puerto de salida del flujo (hacia el servidor 
                   	}                                                             // que ha tocado aleatoriamente)
                 		else							// IP origen impar
                 		{
                 			// Generamos aleatoriamente un número IMPAR entre 0 y num_servers
                 			out_port = (2*uv.GetInteger(0, num_servers)+1)%num_servers;		// Puerto de salida del flujo (hacia el servidor 
                 		}                                                               // que ha tocado aleatoriamente)
			      }
